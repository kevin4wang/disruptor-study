...
//    ringBuffer 容器中的数据存储结构。每一个数据都放在Object数组中
...
//        这里真正创建的ringBuffer的数据存储大小
        this.entries = new Object[sequencer.getBufferSize() + 2 * BUFFER_PAD];
        fill(eventFactory);
    }

//    内存预加载的具体实现；所谓内存预加载是指创建bufferSize个空对象，把对应的内存分配好。另一方面，避免对象的回收。只要ringBuffer一直存在
//    则初始化的对象分配的内存就不会被回收。
 ...
 * disruptor中的环状容器，所有写线程的消息都在该类中进行存储
 */
public final class RingBuffer<E> extends RingBufferFields<E> implements Cursored, EventSequencer<E>, EventSink<E>
{
     ...
     * 获取消费者线程的最小序列，获取这个序列的意义在于写线程生产出来的数据是否能覆盖该序列对应的数据，如果
     * 写线程的序列号大于消费者线程的最小序列，则写线程不能覆盖该最小序列的数据。
     */
